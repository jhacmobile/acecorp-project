                  ...
                  {filteredEmployees.map(e => {
                     ...
                     return (
                        <tr key={e.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td key={idx} className={`p-3 border-l border-slate-100 ${status === 'ABSENT' ? 'opacity-20' : ''}`}>
                                   <div className="flex flex-col items-center gap-2 relative">
                                      <div className="relative">
                                         <select value={status} onChange={(ev) => updateDayStatus(e.id, date, ev.target.value as any)} className="appearance-none w-full text-[9px] font-black uppercase border-none bg-transparent text-center cursor-pointer outline-none hover:text-sky-600">
                           ...
                           <td className="border-l border-slate-100 p-3 bg-rose-100/10"><p className="text-[13px] font-black text-rose-900 italic">₱{calc.utDeduction.toLocaleString()}</p><div className="text-[8px] text-rose-50 font-black tracking-widest mt-1">UT</div></td>
